...
void Mesh::InitializeMesh(const char* meshFile, MPI_Comm comm, int order) {
    ...
    Mesh::InitializeGlobalMesh(Constants::mesh_file);

    // Initialize the parallel mesh
    Mesh::InitializeParallelMesh(MPI_COMM_WORLD);

    // Set up the finite element space
    Mesh::SetupFiniteElementSpace(Constants::order);

    // Set up the parallel finite element space
    Mesh::SetupParFiniteElementSpace(Constants::order);

    // Assign the global values
    Mesh::AssignGlobalValues(Constants::mesh_file);

    // Map the global values to the local
    Mesh::MapGlobalToLocal(Constants::mesh_file);
...
void Mesh::AssignGlobalValues(const char* meshFile) {
    std::string meshFileStr(meshFile);  // Convert to std::string
...
void Mesh::MapGlobalToLocal(const char* meshFile) {
