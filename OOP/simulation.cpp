...
void check_nan(const std::string &name, const mfem::ParGridFunction &gf, int tstep) {
    double local_nan = 0.0;
    ...
    double global_nan = 0.0;
    MPI_Allreduce(&local_nan, &global_nan, 1, MPI_DOUBLE, MPI_MAX, MPI_COMM_WORLD);

    if (global_nan > 0.5) {
        std::cerr << "[Timestep " << tstep << "] NaN detected in " << name << std::endl;
...
int main(int argc, char *argv[]) {
    ...
    particle_concentration.Initialize(CnCH_gf, 0.0202, *domain_parameters.psi);  // initial value: 2.02d-2
