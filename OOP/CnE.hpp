...
public:
    ...
    std::unique_ptr<mfem::ParGridFunction> RxE; ///< Pointer to a grid function storing reaction values for the electrolyte
    ...
    std::shared_ptr<mfem::HypreParVector> CeVn; ///< Variable to store the electrolyte concentration vector at the next time step

private:

    std::shared_ptr<mfem::ParFiniteElementSpace> fespace; ///< Pointer to the finite element space

    std::unique_ptr<mfem::ParGridFunction> PeR; ///< Pointer to a grid function storing reaction potential values
